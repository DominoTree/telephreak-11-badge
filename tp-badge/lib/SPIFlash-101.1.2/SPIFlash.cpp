...
void SPIFlash::command(uint8_t cmd, boolean isWrite){
  ...
  if (isWrite)
  {
    unselect();
    select();
    SPI.transfer(0x50); //enable write to status register
    unselect();
    select();
    SPI.transfer(0x01); //write to status register 1
    SPI.transfer(0x00); //data
    unselect();
    select();
    SPI.transfer(0x06);
    unselect();
    //Serial.println(readStatus()); //this should be 0x08 on a good day
  }
